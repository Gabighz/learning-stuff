/*
    Created as part of the 'Ultimate Rust Crash Course' taught by Nathan Stocks
    on Udemy.
*/
...
fn main() {
    ...
    let handle_a = thread::spawn(move || {
        // originally 0 - now it's 500 which is greater than 100 + 200 from main thread + b thread
        pause_ms(500);
    ...
    // the child threads.
    {
        ...
        let (t, r) = channel::bounded(my_vector.len());
        let (t1, r1) = channel::bounded(my_vector.len());

        let handle_a = thread::spawn(move || {
            while !r.is_empty(){
                println!("Thread A received: {}", r.recv().unwrap());
            }
        });

        let handle_b = thread::spawn(move || {
            while !r1.is_empty(){
                println!("Thread B received: {}", r1.recv().unwrap());
            }
        });

        for value in my_vector {
            println!("Sending to a: {}", value);
            t.send(value).unwrap();
            println!("Sending to b: {}", value);
            t1.send(value).unwrap();
        }
